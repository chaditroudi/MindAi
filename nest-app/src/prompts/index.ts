...
const CHART_DATA_MAX_CHARS = Number(
  process.env['CHART_DATA_MAX_CHARS'] ?? 30_000,
);
const CHART_STYLE_CONTEXT =
  process.env['CHART_STYLE_CONTEXT'] ??
  [
...
function profileColumns(rows: Record<string, unknown>[], source?: DataSource) {
  ...
  return [...keys].map((k) => {
    ...
    const schema = source?.fields.find(
      (f) => f.name.toLowerCase() === k.toLowerCase(),
    );
    return {
      name: k,
...
export function buildChartPrompt(
  ...
  const values: Record<string, string> = {
    ...
    '{{SOURCE_DESCRIPTION}}':
      source?.description ?? 'No source description provided.',
  ...
  if (source?.suggestedCharts?.length) {
    const hints = source.suggestedCharts.map(
      ({ pipeline: _p, ...rest }) => rest,
    );
    return (
