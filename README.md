...
**What it costs to run:** Every question costs **two AI calls** (one to plan the database query, one to write the final answer) — cheaper when a follow-up question can reuse the previous plan, and free (zero AI calls) when an identical question was asked in the last 7 days and the result is still cached. Token usage per user/connection is tracked automatically ([§8](#8-api-reference) `GET /api/settings`, [§7](#7-data-model--mongodb-collections) `agent_config`) so cost is attributable. A **third, optional AI call** runs in the background after most answers to extract long-term memory (see [§6](#6-ai-pipeline-in-detail)) — small, but real token cost not included in the "two calls" figure above; it can be turned off via `PATCH /api/memory/config`.
...
### Long-term memory — extraction, dedup, and recall (`ai/memory-skill.ts`, `memory/`)

Long-term memory is a loop across **two separate requests**, not one: what gets extracted after *this* answer only affects the *next* prompt, never the current one.

**1. Extraction — end of the current request, LLM call, best-effort.** After a response is built, `AnalyticsService.maybeExtractMemory()` fires `extractMemories(prompt, responseSummary, ...)` if the response summary is longer than `MIN_SUMMARY_LENGTH_FOR_MEMORY` (30 characters) — this call happens after the client has already received its response conceptually, but is currently awaited via a fire-and-forget `void` call, not truly backgrounded off the request lifecycle. It asks the model (role `memory`, its own `skills/memory/SKILL.md` prompt) to extract **at most 3** memory items, each: `type` (`goal|insight|preference|context|decision|entity|correction`), `content` (5–300 characters), up to 5 `tags`, and an `importance` 1–5. A failure here is swallowed and logged, never surfaced to the user.

**2. De-duplication — `MemoryRepository.upsert()`, word-overlap similarity.** Before inserting a newly extracted item, it's compared against the user's existing memories of the same `type` by **Jaccard similarity over significant words** (content lowercased, split on non-alphanumerics, stopwords like "show"/"for"/"all"/"list" removed). If the best-matching existing memory scores at or above `MEMORY_DEDUPE_SIMILARITY_THRESHOLD` (default `0.5`), it's updated in place (content/tags/importance/sessionId) instead of creating a duplicate. This is a similarity match on purpose, not an exact one — natural rephrasing shouldn't fragment into separate memories:

  | Pair | Shared significant words | Score | Outcome |
  |---|---|---|---|
  | "show budget vs duration for **all** projects" vs "...for projects" | `{budget, duration, projects}` vs `{budget, duration, projects}` | `1.00` | merge |
  | "...for all projects" vs "...for projects **of 4 municipalities**" | `{budget, duration, projects}` vs `{budget, duration, projects, municipalities}` | `0.75` | merge |
  | "...for all projects" vs "list all **cancelled** projects" | `{budget, duration, projects}` vs `{cancelled, projects}` | `0.25` | stays separate |

**3. Recall & injection — start of the *next* request.** `AnalyticsService.buildMemoryContext()` calls `MemoryService.getRelevantContext()` → `MemoryRepository.findRelevant()`, which pulls the user's **top 100** memories by `importance` (desc) then recency, re-scores only that pool by tag-overlap with words in the new prompt plus a small importance bonus, and returns the top `memoryLimit` (per-agent config, default 50 — [§7](#7-data-model--mongodb-collections)). The result is formatted as `[GOAL] ...` / `[INSIGHT] ...` lines and injected as a synthetic user/assistant message pair **ahead of the planner call**, so the model reads it as context the user already provided. Because relevance re-ranking only ever runs over that top-100 pre-filter, a memory that would score well on tag-overlap but falls outside the top 100 by importance/recency is never evaluated at all — this is a two-stage funnel, not a search over everything ever stored for that user.

`GET/PATCH /api/memory/config` toggles extraction, but it is a **single global switch** — it is not scoped per `x-user-id` like every other memory endpoint (`GET`/`DELETE /api/memory`), so any caller can turn memory extraction off for every user in the system.
...
```
`findRelevant()` is a lightweight keyword/tag overlap scorer (`hits/words + importance/20`) over the user's most recent 100 items — not a vector/embedding search despite the field name suggesting otherwise. `upsert()` de-duplicates by Jaccard word-overlap similarity over each content's significant words (stopwords stripped) against every existing memory of the same `userId`+`type`, updating the best match in place (content/tags/importance/sessionId) once similarity reaches `MEMORY_DEDUPE_SIMILARITY_THRESHOLD` (default `0.5`) instead of creating a duplicate — see [§6](#6-ai-pipeline-in-detail) for the exact algorithm and worked examples. `memory_settings` holds a single global `extractionEnabled` boolean toggled via `PATCH /api/memory/config` — a system-wide switch, **not per-user**, unlike every other row in this table.
...
Not MongoDB — a local SQLite file managed by `@mastra/memory`/`@mastra/libsql`. One thread per `sessionId`, with `title`, `metadata.intent`, and up to the last N messages retrievable as `CoreMessage[]` via `getMemoryContext()`. Each assistant message embeds the full `MessageResult` (`dashboardSpec` / `reportSections` / `summary` + `durationMs`) in `metadata.uiMessage` so the history endpoints can reconstruct rich turns. Every thread operation (`sessionExists`, `ensureThread`, `getMemoryContext`, `listSessions`, `getSessionDetail`, `deleteSession`) passes the caller's `x-user-id` as Mastra's `resourceId`, which the storage layer filters on — a thread created under one `x-user-id` is invisible to a request made under a different one. This puts conversation history under the same trust boundary as everything else in [§15](#15-security-model--trust-boundaries): scoped by `x-user-id`, and `x-user-id` itself remains client-supplied and unverified.
...
| Long-term memory extraction & recall | ⚠️ Works, with caveats | Relevance ranking is keyword/tag overlap, not embeddings, over only the top 100 memories by importance/recency; extraction is `void`-called, not truly decoupled from the request; the enable/disable toggle is global, not per-user |
...
- **Long-term memory recall only searches the top 100 memories, not all of them** ([§6](#6-ai-pipeline-in-detail)) — `findRelevant()` pre-filters to the 100 highest-importance/most-recent memories before scoring tag overlap against the current prompt. A memory that would be highly relevant by tags but is old and was extracted with low importance can fall outside that pre-filter and never be considered, no matter how well it would otherwise match.
- **Memory extraction's on/off switch is global, not per-user** — `GET`/`PATCH /api/memory/config` reads/writes a single `memory_settings` document with no `x-user-id` scoping, unlike `GET`/`DELETE /api/memory` in the same controller. Any caller can disable memory extraction for every user in the system with one request.
- **Nothing prunes long-term memory automatically** — `memory_items` only ever grows; the sole cleanup path is a user manually hitting `DELETE /api/memory` ("Clear all" in the UI). There is no TTL, item cap, or archival policy.
