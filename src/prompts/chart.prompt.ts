...
import type { DataSource, SkillKind, ChartHint } from '../types/index.js';

const INSTRUCTIONS = readMarkdownSection(
  new URL('../../skills/chart/SKILL.md', import.meta.url),
  'Runtime Prompt',
);

const PROFILE_ROWS = Number(process.env.CHART_PROFILE_ROWS ?? 10);
const SAMPLE_ROWS  = Number(process.env.CHART_SAMPLE_ROWS  ?? 3);
